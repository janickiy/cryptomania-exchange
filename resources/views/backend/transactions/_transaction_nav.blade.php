...
<div class="row dc-clear">
    ...
    @foreach(config('commonconfig.journal_type') as $key => $value)
        ...
        <div class="col-md-3 col-sm-6 cm-mb-5">
            <a data-title="{{ \Illuminate\Support\Str::title(str_replace('-',' ', $key)) }}" data-toggle="tooltip" class="btn btn-block btn-default {{ is_current_route($routeName, 'active', ['journalType' => $key]) }}" href="{{ route($routeName, $parameters) }}" style="text-overflow: ellipsis; overflow: hidden">{{ \Illuminate\Support\Str::title(str_replace('-',' ', $key)) }}</a>
        </div>
    @endforeach
</div>
