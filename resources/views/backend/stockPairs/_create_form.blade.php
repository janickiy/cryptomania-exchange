...
<div class="admin-form-section">
    ...
    <div class="admin-form-grid">
        ...
        <div class="admin-form-field {{ $errors->has('is_default') ? 'has-error' : '' }}">
            <label for="{{ fake_field('is_default') }}" class="form-label required">{{ __('Is Default') }}</label>
            <div class="cm-switch stock-switch">
                {{ Form::radio(fake_field('is_default'), ACTIVE_STATUS_ACTIVE, false, ['id' => fake_field('is_default') . '-active', 'class' => 'cm-switch-input', 'data-cval-name' => 'Is default field', 'data-cval-rules' => 'required|integer|in:' . array_to_string(active_status())]) }}
...
<div class="admin-form-actions">
    ...
    <a href="{{ route('admin.stock-pairs.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
</div>
