...
<section class="admin-form-section">
    <div class="admin-form-section-header">
        <h4 class="admin-form-section-title">{{ __('Post Content') }}</h4>
    </div>
    ...
    <div class="admin-form-grid">
        ...
        <div class="admin-form-field admin-form-field-wide {{ $errors->has('content') ? 'has-error' : '' }}">
            <label for="content_textarea" class="form-label required">{{ __('Content') }}</label>
...
<section class="admin-form-section">
    <div class="admin-form-section-header">
        <h4 class="admin-form-section-title">{{ __('Media and Status') }}</h4>
    </div>

    <div class="admin-form-grid">
        <div class="admin-form-field admin-form-field-wide {{ $errors->has('featured_image') ? 'has-error' : '' }}">
            <label for="{{ fake_field('featured_image') }}" class="form-label required">{{ __('Featured Image') }}</label>
            <div class="fileinput fileinput-new trade-analysis-fileinput" data-provides="fileinput">
                <div class="fileinput-new thumbnail trade-analysis-thumbnail">
                    @if(isset($post))
                ...
                <div class="fileinput-preview fileinput-exists thumbnail trade-analysis-thumbnail"></div>
                <div class="trade-analysis-file-actions">
                    ...
                    <a href="javascript:;" class="btn btn-outline-secondary fileinput-exists" data-dismiss="fileinput">{{ __('Remove') }}</a>
                </div>
            </div>
            <p class="form-text">{{ __('Upload item emoji 400X400 and less than or equal 512KB.') }}</p>

            <span class="validation-message cval-error" data-cval-error="{{ fake_field('featured_image') }}">{{ $errors->first('featured_image') }}</span>
        </div>

        <div class="admin-form-field {{ $errors->has('is_published') ? 'has-error' : '' }}">
            <label for="{{ fake_field('is_published') }}" class="form-label required">{{ __('Status') }}</label>
            <div class="cm-switch trade-analysis-switch">
...
<div class="admin-form-actions">
    ...
    <a href="{{ route('trade-analyst.posts.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
</div>
